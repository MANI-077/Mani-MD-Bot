...
const GITHUB_TOKEN = process.env.GITHUB_TOKEN || settings.githubToken;
...
async function uploadSession() {
    try {
        ...
        // Relaxed check - only verify it's valid JSON with some content
        // (During pairing, creds may not have noiseKey yet, but we still want to save state)
        try {
            const parsed = JSON.parse(content);
            // Only skip if the file is essentially empty (just {})
            if (Object.keys(parsed).length === 0) {
                console.log('⚠️ [SESSION-SYNC] Empty creds.json, skipping upload.');
                return;
            }
        } catch (e) {
